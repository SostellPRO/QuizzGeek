...
const TABS = [
  { id: 'pilotage', labelKey: 'host.tab.control', icon: '⚡' },
  { id: 'gestion', labelKey: 'host.tab.players', icon: '👥' },
];
...
export default function HostGame() {
  const { gameState: gs, players, hostSession, setHostSession, navigate, t } = useGame();
        ...
        <div className="mx-auto flex max-w-5xl flex-col gap-3 px-4 py-3">
          <div className="flex items-center justify-between gap-3">
            <div className="flex min-w-0 items-center gap-3">
              <button onClick=${disconnect} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg app-chip text-white/58 transition-colors hover:text-white" title="Quitter">
                ←
              </button>
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="rounded-full app-chip px-2.5 py-1 text-xs font-bold uppercase tracking-[0.14em] text-white/42">${t('common.session')}</span>
                  <span className="font-mono text-sm font-black tracking-widest text-sky-300">${hostSession?.sessionCode}</span>
                  <span className="rounded-full bg-teal-400/12 px-2.5 py-1 text-xs font-bold text-teal-200">${PHASE_LABEL_KEYS[phase] || phase}</span>
                </div>
                <div className="mt-1 truncate text-sm text-white/48">
                  ${gs?.quizTitle || t('host.waitingQuiz')}
                </div>
              </div>
            </div>
            ...
            <div className="hidden grid-cols-2 gap-2 sm:grid">
              <div className="rounded-lg app-panel px-3 py-2 text-right">
                <div className="text-xs font-semibold text-white/38">${t('common.players')}</div>
                <div className="font-mono text-lg font-black text-teal-300">${connectedPlayers}</div>
              </div>
              <div className="rounded-lg app-panel px-3 py-2 text-right">
                <div className="text-xs font-semibold text-white/38">${t('common.bots')}</div>
          ...
          <div className="grid grid-cols-2 gap-2 rounded-lg app-panel p-1">
            ${TABS.map(tabDef => html`
              <button
                key=${tabDef.id}
                onClick=${() => setTab(tabDef.id)}
                className=${`ui-btn flex min-h-[42px] items-center justify-center gap-2 rounded-lg text-sm font-extrabold ${
                  tab === tabDef.id
